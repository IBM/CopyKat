...
def __replay(rootfs, kernel, record, ignored_addresses, func_map, symbol_map,
	...
	outfile_file = open('./kllvm_output2', "w+")
	tb_file = open('./kllvm_tb_output2.txt', "w+")
	...
	def on_call(cpu, addr):
		...
		##	  # log(f'f: __kdo_asan_memmove, dest: {hex(dest)}, src: {hex(src)}, length: {len}, call_id: {call_id}, pc: {hex(get_retval(cpu))}', file=outfile_file)
			...
			if call_id == sink_call_id:
				...
				for offset in range(size):
					...
					if taint is None:
						...
						log(f'label on soruce ptr at offset {offset} are {labels}', file=outfile_file)
				...
				analysis["taint_src"] = taint_info

				print("Terminating analysis...")
				panda.end_analysis()

		elif addr == kdo_copy_from_user_addr:
			...
			if cfu_id not in taint_sites:
				taint_sites[cfu_id] = 0
			else:
				taint_sites[cfu_id] += 1
